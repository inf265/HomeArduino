...
Info INFOS[NUM_LEDS] = {
  {9, 38, LED_OFF},
  {21, 43, LED_OFF},
  {11, 34, LED_OFF}, // Guest WC
  {17, 35, LED_OFF},
  {20, 44, LED_OFF},
  {10, 40, LED_OFF} // esstisch
};
...
void setup() {
  ...
  for (uint8_t i = 0; i < NUM_LEDS; i++) {
    ...
    digitalWrite(INFOS[i].ledPin, HIGH);
...
void handleEvent(AceButton* button, uint8_t eventType, uint8_t buttonState) {
  ...
  // Print out a message for all events.
  Serial.print("handleEvent(): eventType: ");
  Serial.print(eventType);
  ...
  switch (eventType) {
    case AceButton::kEventPressed:
      bool etat = digitalRead(ledPin);
      //INFOS[id].ledState = !etat;
      Serial.println("Pressed");
      digitalWrite(INFOS[id].buttonPin, !etat);
      break;
